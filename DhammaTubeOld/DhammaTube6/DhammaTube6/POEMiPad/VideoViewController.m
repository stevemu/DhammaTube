...
- (void)viewDidLoad
{
    ...
    NSString *tempvideoid = [GlobalData currentVideoId];
    ...
    YoutubeAPICommunicator *yt = [[YoutubeAPICommunicator alloc] init];
}

-(void)loadVideoWithVideoId:(NSString *)videoId {

    self.video = [Video MR_findFirstByAttribute:@"video_id" withValue:videoId];

    if (!self.video) {
        self.video = [Video MR_createEntity];
        self.video.video_id = videoId;
        self.video.time = @0;
    }
